...
#  define BX_RADEON_SMF  static
#  define BX_RADEON_THIS theSvga->
#  define BX_RADEON_THIS_PTR theSvga
#else
#  define BX_RADEON_SMF
#  define BX_RADEON_THIS this->
#  define BX_RADEON_THIS_PTR this
#endif // BX_USE_RADEON_SMF

// 0x3b4,0x3d4
#define VGA_CRTC_MAX 0x18
#define RADEON_CRTC_MAX 0x3f
// 0x3c4
#define VGA_SEQENCER_MAX 0x04
#define RADEON_SEQUENCER_MAX 0x1f
// 0x3ce
#define VGA_CONTROL_MAX 0x08
#define RADEON_CONTROL_MAX 0x39

// Size of internal cache memory for bitblt. (must be >= 256 and 4-byte aligned)
#define RADEON_BLT_CACHESIZE (2048 * 4)

#define RADEON_VIDEO_MEMORY_MB    128
...
private:
...
#endif
  BX_RADEON_SMF void   svga_init_members();

  BX_RADEON_SMF void draw_hardware_cursor(unsigned, unsigned, bx_svga_tileinfo_t *);

  // 0x3b4-0x3b5,0x3d4-0x3d5
  BX_RADEON_SMF Bit8u svga_read_crtc(Bit32u address, unsigned index);
  BX_RADEON_SMF void  svga_write_crtc(Bit32u address, unsigned index, Bit8u value);
  ...
  struct {
    Bit8u index;
    Bit8u reg[RADEON_SEQUENCER_MAX+1];
  } sequencer; // 0x3c4-5
  struct {
    Bit8u index;
    Bit8u reg[RADEON_CONTROL_MAX+1];
    Bit8u shadow_reg0;
    Bit8u shadow_reg1;
  } control; // 0x3ce-f
  struct {
  ...
  struct {
    ...
    int memdst_needed;
    Bit8u memsrc[RADEON_BLT_CACHESIZE];
    Bit8u memdst[RADEON_BLT_CACHESIZE];
  } bitblt;
